        ...
        stateMessages.map((message: any) => {
          return (
            <div
              ...
              ref={messageEndRef}
            >
              <span className={`text-white chat-bubble ${message.senderId === props.userId ? "bg-purple-600" : "bg-zinc-500"}`}>{message.text}</span>
            </div>
          )
        })
      }
...
function ChatContainer() {
  ...
  const { loading: queryLoading, data, error: queryError, refetch } = useQuery(getMessagesQuery, { variables: { receiverId } , context: userContext, skip: !receiverId || !getLocalStorage("token") })
  ...
  return (
    <div className="absolute inset-x-[45vh] ml-[10vh] my-[2vh] h-[98%] flex flex-col bg-zinc-200 rounded-xl">
